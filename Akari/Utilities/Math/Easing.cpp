...
namespace Math
{
	...
	double easeInElastic(double t) {
		double t2 = t * t;
		return t2 * t2 * stdc::f_sinf(t * PI * 4.5);
	}

	double easeOutElastic(double t) {
		double t2 = (t - 1) * (t - 1);
		return 1 - t2 * t2 * stdc::f_cosf(t * PI * 4.5);
	}
	...
	double easeInOutElastic(double t) {
		...
		if (t < 0.45) {
			t2 = t * t;
			return 8 * t2 * t2 * stdc::f_sinf(t * PI * 9);
		}
		else if (t < 0.55) {
			return 0.5 + 0.75 * stdc::f_sinf(t * PI * 4);
		}
		else {
			t2 = (t - 1) * (t - 1);
			return 1 - 8 * t2 * t2 * stdc::f_sinf(t * PI * 9);
		}
	}

	double easeInBounce(double t) {
		return stdc::pow(2, 6 * (t - 1)) * stdc::fabs(stdc::f_sinf(t * PI * 3.5));
	}

	double easeOutBounce(double t) {
		return 1 - stdc::pow(2, -6 * t) * stdc::fabs(stdc::f_sinf(t * PI * 3.5));
	}

	double easeInOutBounce(double t) {
		if (t < 0.5) {
			return 8 * stdc::pow(2, 8 * (t - 1)) * stdc::fabs(stdc::f_sinf(t * PI * 7));
		}
		else {
			return 1 - 8 * stdc::pow(2, -8 * t) * stdc::fabs(stdc::f_sinf(t * PI * 7));
		}
	}
