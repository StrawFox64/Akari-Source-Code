...
namespace Memory
{
	...
	{
		std::memset(m_TrampolineOpd, 0, sizeof(m_TrampolineOpd));
		std::memset(m_OriginalInstructions, 0, sizeof(m_OriginalInstructions));
	}

	Detour::Detour(uint32_t fnAddress, uint32_t fnCallback)
		: m_HookTarget(nullptr), m_HookAddress(nullptr), m_TrampolineAddress(nullptr), m_OriginalLength(0)
	{
		std::memset(m_TrampolineOpd, 0, sizeof(m_TrampolineOpd));
		std::memset(m_OriginalInstructions, 0, sizeof(m_OriginalInstructions));
	...
	size_t Detour::JumpWithOptions(void* destination, const void* branchTarget, bool linked, bool preserveRegister, uint32_t branchOptions, uint8_t conditionRegisterBit, uint8_t registerIndex)
	{
		...
		if (destination)
			WriteProcessMemory(sys_process_getpid(), (uint32_t)destination, BranchAsm, BranchAsmSize);
	...
	size_t Detour::RelocateBranch(uint32_t* destination, uint32_t* source)
	{
		...
		{
			WriteProcessMemory(sys_process_getpid(), (uint32_t)destination, &Instruction, sizeof(Instruction));
	...
	void Detour::Hook(uint32_t fnAddress, uint32_t fnCallback, uint32_t tocOverride)
	{
		m_HookAddress = reinterpret_cast<void*>(fnAddress);
		m_HookTarget = reinterpret_cast<void*>(*reinterpret_cast<uint32_t*>(fnCallback));
		...
		WriteProcessMemory(sys_process_getpid(), (uint32_t)m_OriginalInstructions, m_HookAddress, HookSize);
		...
		m_TrampolineOpd[1] = tocOverride != 0 ? tocOverride : Memory::GetCurrentToc();
	}
	...
	bool Detour::UnHook()
	{
		...
		{
			WriteProcessMemory(sys_process_getpid(), (uint32_t)m_HookAddress, m_OriginalInstructions, m_OriginalLength);
